...
public class FTPTransfer implements FileTransfer {
    ...
    private List<FileInfo> getListing(final String path, final int depth, final int maxResults) throws IOException {
        ...
        final int transferWait = ctx.getProperties().containsKey(FTPTransfer.TRANSFER_WAIT) ? ctx.getProperty(FTPTransfer.TRANSFER_WAIT).asInteger() : 0;

        final List<FileInfo> listing_pre = getListing(path,depth,maxResults,pathPattern,remotePath,ignoreDottedFiles,recurse,pattern);

        if(transferWait > 0) {
            try {
                Thread.sleep(transferWait * 1000);
            } catch (InterruptedException ex) {
            }

            final List<FileInfo> listing_post = getListing(path,depth,maxResults,pathPattern,remotePath,ignoreDottedFiles,recurse,pattern);
            Map<String,Long> postMap=new HashMap<>();
            for(FileInfo post : listing_post) {
                postMap.put(post.getFullPathFileName(), post.getSize());
            }

            List<FileInfo> listing = new ArrayList<>();

            for(FileInfo pre : listing_pre) {
                Long size=postMap.get(pre.getFullPathFileName());
                if(size != null && pre.getSize() == size) {
                    listing.add(pre);
                }
            }
            return listing;
    ...
    private List<FileInfo> getListing(final String path, final int depth, final int maxResults, final Pattern pathPattern, final String remotePath, final boolean ignoreDottedFiles, final boolean recurse, final Pattern pattern) throws IOException {
        ...
        for (final FTPFile file : files) {
            ...
            if (recurse && file.isDirectory()) {
                try {
                    listing.addAll(getListing(newFullForwardPath, depth + 1, maxResults - count, pathPattern, remotePath, ignoreDottedFiles, recurse, pattern));
